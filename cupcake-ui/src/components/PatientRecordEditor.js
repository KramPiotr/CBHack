 ...
 */
class PatientRecordEditor extends Component {
  onChange = (fn) => {
    const { patientRecord, onChange } = this.props;
    const patientRecordJSON = JSON.parse(JSON.stringify(patientRecord)); // Hack for deep copy

    return (evt) => {
      const value = evt.target.type === 'checkbox' ? evt.target.checked : evt.target.value;

      fn(patientRecordJSON, value); // Update cupcake

      if (onChange) {
        onChange(patientRecordJSON);
      }
    };
  };

  onBake = (evt) => {
    const { patientRecord, onBake } = this.props;
    evt.preventDefault();

    console.log('making patientRecord..');

    createFile(patientRecord).then(data => console.log(data));

    if (onBake) {
      onBake(patientRecord);
    }
  };

  render() {
    const {
      patientRecord, flavourColours, icingColours, sprinklesColours, candleColours,
    } = this.props;
